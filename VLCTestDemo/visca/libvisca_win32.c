...
VISCA_open_serial(VISCAInterface_t *iface, const char *device_name)
{
  ...
  // Port settings are specified in a Data Communication Block (DCB). The easiest way to initialize a DCB is to call GetCommState to fill in its default values, 
  ...
  m_dcb.fRtsControl = RTS_CONTROL_ENABLE;   // Disable RTS (Ready To Send)
  m_dcb.fOutX = FALSE;			     // Disable XON/XOFF for transmission
  m_dcb.fInX = FALSE;			     // Disable XON/XOFF for receiving
  m_dcb.fBinary = TRUE;			     // Must be always TRUE!
